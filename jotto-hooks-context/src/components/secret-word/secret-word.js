...
/**
 * The component shows  panes of secretWord letters
...
const SecretWord = ({ level, secretWord }) => {
  // initially fill in the panes of secret word with spaces
  ...
  /**
   * Effect to set success when the word is guessed
   */
  React.useEffect(() => {
  ...
  /**
   * Effect to open the guessed letters
   */
  React.useEffect(() => {
    ...
    switch (level) {
      ...
      case 'medium':
        ...
        break;
      case 'hard':
        // show all letters when the whole word is guessed
        if (guessedWord === secretWord) {
          setLetters(guessedWord.split(''));
        }
        break;
